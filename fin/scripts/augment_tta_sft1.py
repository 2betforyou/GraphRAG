...
Label balancing:
  --balance_labels 를 주면, 보기 개수별로 정답 라벨(1..N)을 라운드로빈으로 균등 분포시킴.
...
Usage:
  python scripts/augment_tta_sft1.py \
...
class LabelBalancer:
    """
    보기 개수(n_options) 별로 정답 라벨(1..n_options)을 라운드로빈으로 배분.
    예) 4지선다면 1,2,3,4,1,2,3,4,... 순으로 정답 위치를 목표로 배정.
    """
    def __init__(self):
...
def place_target_at(choices: List[str], target_value: str, target_pos_1based: int) -> Tuple[List[str], int]:
    """
    choices 내 target_value가 존재한다고 가정.
    target_value를 target_pos_1based 위치로 스왑 이동.
    반환: (수정된 choices, 정답 1-based 인덱스)
    """
...
# ----- MCQ 기록 생성 -----
def make_mcq_def_normal(rows, X, i, n_options=4, balancer: Optional[LabelBalancer]=None, balance=False) -> Dict[str,str]:
    """ 용어 -> 올바른 정의 선택 (정상형) """
    term = rows[i]["term"]
    correct_def, choices, ans_1based, _ = _build_choices_from_defs(rows, i, X, n_options)
...
def make_mcq_def_neg(rows, X, i, n_options=4, balancer: Optional[LabelBalancer]=None, balance=False) -> Dict[str,str]:
...
def make_mcq_term_normal(rows, X, i, n_options=4, balancer: Optional[LabelBalancer]=None, balance=False) -> Dict[str,str]:
    """ 정의 -> 올바른 용어 선택 (정상형) """
    definition = shorten(rows[i]["definition"])
    correct_term, choices, ans_1based, _ = _build_choices_from_terms(rows, i, X, n_options)
    if balance and balancer:
        target_pos = balancer.next_pos(n_options)
...
def make_mcq_term_neg(rows, X, i, n_options=4, balancer: Optional[LabelBalancer]=None, balance=False) -> Dict[str,str]:
...
def main():
    ...
    for i in range(len(rows)):
        for n_opt in args.options:
            # A) term -> 정의
            rec = make_mcq_def_normal(rows, X, i, n_options=n_opt, balancer=balancer, balance=balance)
            aug.append(rec)
            if args.add_neg:
                rec_neg = make_mcq_def_neg(rows, X, i, n_options=n_opt, balancer=balancer, balance=balance)
                aug.append(rec_neg)

            if args.mcq_per_item >= 2:
                # B) 정의 -> 용어
                rec2 = make_mcq_term_normal(rows, X, i, n_options=n_opt, balancer=balancer, balance=balance)
                aug.append(rec2)
                if args.add_neg:
                    rec2_neg = make_mcq_term_neg(rows, X, i, n_options=n_opt, balancer=balancer, balance=balance)
                    aug.append(rec2_neg)
