...
- 주관식: 한두 문장으로 간결 생성 (beam=4, do_sample=False 유지)
...
def encode_chat(tokenizer, model, messages, want_new_tokens: int, reserve: int = 32):
    ...
    - add_generation_prompt=True로 템플릿 적용 (주관식 품질 유지)
    """
...
# --- 주관식 출력 클리닝(에코/역할태그 제거) ---
ROLE_HEAD_RE = re.compile(r"^(assistant|assistant:)\s*", flags=re.I)

def clean_subjective_answer(s: str, q: str) -> str:
    s = s.strip()
    s = ROLE_HEAD_RE.sub("", s)  # 'assistant ' 제거
    # 질문 에코가 선두에 붙는 경우 억제(질문 문장 일부가 그대로 시작될 때)
    head = q.strip().splitlines()[0]
    if len(head) >= 6 and s.startswith(head[: max(6, len(head)//2)]):
        s = s[len(head)//2:].lstrip()
    # 과한 공백/기호 정리
    s = re.sub(r"\s+", " ", s)
    s = re.sub(r"\.{2,}", "…", s)
    # 2문장 정도로 컷(너무 장황한 경우 유사도에 노이즈)
...
def main():
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Infer"):
        ...
        # (선택) RAG 문맥을 “텍스트 뒤에” 붙여서 make_prompt_auto로
        ...
        if is_multiple_choice(full_text_for_prompt):
            ...
            circled_map = {"①":"1","②":"2","③":"3","④":"4","⑤":"5","⑥":"6",
                        "⑦":"7","⑧":"8","⑨":"9","⑩":"10","⑪":"11","⑫":"12"}
            _, options = extract_question_and_choices(full_text_for_prompt)
            opt_text = "\n".join(options)
            ...
            processors = LogitsProcessorList([FirstDigitOnly(tok, nums)])
            ...
            # ---------------- 주관식: 품질 유지(beam=4, deterministic) + 에코/잡음 클리닝 ----------------
            ...
            ans = clean_subjective_answer(raw_ans, full_text)  # 에코/역할/장황함 정리
            preds.append(ans)
