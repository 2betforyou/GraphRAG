...
"""
import argparse, json, re, datetime, textwrap
...
def main():
    ...
    def run_one(q_text: str):
        ...
        if is_mc and args.mcq_explain:
            ...
            logits_proc = LogitsProcessorList([FirstFromOptionsOnly(tok, allowed_digits)])
            allowed_token_ids = []
            ...
            messages_std = messages
            inputs = build_chat_inputs(tok, messages_std).to(model.device)
            ...
            if is_mc:
                ...
                logits_proc.append(FirstFromOptionsOnly(tok, allowed_digits))
                gen_kwargs.update(
                    max_new_tokens=4,
                    temperature=0.0, top_p=1.0,
    ...
    if args.test_csv:
        ...
        for q in df["Question"].tolist():
            ...
            # 제출 형식: MCQ면 숫자만, 주관식은 그대로 텍스트 → 대회 포맷에 맞게 조정
